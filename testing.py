...
def main(page: ft.Page):
    ...
    def fixture_and_results_view():
        ...
        def initialize_points_table():
            for tid in team_ids:
                ...
                if not cursor.fetchone():
                    cursor.execute("INSERT INTO points_table (team_id, tournament_id, team_name, games_played, wins, losses, draws, points) "
                                   "SELECT team_id, %s, team_name, 0, 0, 0, 0, 0 FROM teams WHERE team_id = %s", (tournament_id, tid))
            conn.commit()
        ...
        def load_fixtures():
            ...
            for match in matches:
                fixtures_display.controls.append(ft.Text(f"{match['team1']} vs {match['team2']}"))
            page.update()

        def update_points(team_id, result):
            if result == 'win':
                cursor.execute("UPDATE points_table SET games_played = games_played + 1, wins = wins + 1, points = points + 3 WHERE team_id = %s", (team_id,))
            elif result == 'loss':
                cursor.execute("UPDATE points_table SET games_played = games_played + 1, losses = losses + 1 WHERE team_id = %s", (team_id,))
            elif result == 'draw':
                cursor.execute("UPDATE points_table SET games_played = games_played + 1, draws = draws + 1, points = points + 1 WHERE team_id = %s", (team_id,))
            conn.commit()

        def enter_results(e):
            cursor.execute("""
                SELECT m.match_id, m.team1_id, m.team2_id, t1.team_name AS team1, t2.team_name AS team2
                FROM matches m
                JOIN teams t1 ON m.team1_id = t1.team_id
                JOIN teams t2 ON m.team2_id = t2.team_id
                WHERE m.tournament_id = %s
            ...
            if not matches:
                show_snack("No matches found.")
                return

            match_index = 0
            current_match = matches[match_index]

            match_text = ft.Text(f"{current_match['team1']} vs {current_match['team2']}")
            result_dropdown = ft.Dropdown(
                label="Result for Team 1",
                options=[
                    ft.dropdown.Option("win"),
                    ft.dropdown.Option("loss"),
                    ft.dropdown.Option("draw")
                ],
                value="win"
            )
            submit_button = ft.ElevatedButton(text="Submit Result")

            def submit_result(ev):
                nonlocal match_index, current_match
                selected = result_dropdown.value
                t1_id = current_match['team1_id']
                t2_id = current_match['team2_id']

                if selected == "win":
                    update_points(t1_id, "win")
                    update_points(t2_id, "loss")
                elif selected == "loss":
                    update_points(t1_id, "loss")
                    update_points(t2_id, "win")
                elif selected == "draw":
                    update_points(t1_id, "draw")
                    update_points(t2_id, "draw")

                match_index += 1
                if match_index < len(matches):
                    current_match = matches[match_index]
                    match_text.value = f"{current_match['team1']} vs {current_match['team2']}"
                    result_dropdown.value = "win"
                    page.update()
                else:
                    match_text.value = "All match results entered."
                    result_dropdown.visible = False
                    submit_button.visible = False
                    page.update()

            submit_button.on_click = submit_result
            fixtures_display.controls.clear()
            fixtures_display.controls.extend([match_text, result_dropdown, submit_button])
            page.update()
        ...
        return ft.View(
            ...
            controls=[
                ...
                ft.ElevatedButton("Generate Fixtures", on_click=lambda e: generate_round_robin() if current_tournament_type == "round_robin" else generate_knockout()),
                fixtures_display,
                ft.ElevatedButton("Enter Results", on_click=enter_results),
                ft.TextButton("Back to Dashboard", on_click=lambda _: page.go("/tournament"))
            ]
        )
    ...
    def points_table_view():
        ...
        def view_points_table(e):
            ...
            cursor.execute("SELECT * FROM points_table WHERE tournament_id = %s", (tournament_id,))
            for row in cursor.fetchall():
                points_display.controls.append(
                    ft.Text(
                        f"{row['team_name']} - Played: {row['games_played']} | Wins: {row['wins']} | "
                        f"Losses: {row['losses']} | Draws: {row['draws']} | Points: {row['points']}"
                    )
                )
            page.update()
        ...
        return ft.View(
            ...
            controls=[
                ...
                ft.Row([
                    ...
                    ft.ElevatedButton("Reset Points", on_click=reset_points, bgcolor=ft.colors.RED_400, color=ft.colors.WHITE)
                ]),
                points_display,
                ft.TextButton("Back to Dashboard", on_click=lambda _: page.go("/tournament"))
            ]
